...
const LiveManagement = forwardRef<LiveManagementHandle, LiveManagementProps>(
  function LiveManagement({ onOpenAddToRules }, ref) {
    ...
    const seedThemeSmokeState = useCallback(async () => {
      ...
      const firstGroup = groupedWindows
        .flatMap((win) => win.groups)
        .find((group) => group.tabs.length > 0)
      ...
      >
        <div className="relative flex flex-col gap-6 p-2 pb-6 min-h-full">
          {renderedAutoGroupScanStatus?.message && (
            <div
              className={cn('pointer-events-none fixed inset-x-0 z-40 flex justify-center px-2')}
              ...
              >
                <div className="flex items-center gap-2">
                  <Monitor
                    size={12}
                    className={
                      win.isCurrent ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)]'
                    }
                  />
                  <p
                    className={cn(
                                  ...
                                  {isNamingNewSnapshot && (
                                    <div className="sp-subtle-surface flex flex-col gap-1.5 rounded-lg p-1.5">
                                      <input
                                        ...
                                        onChange={(event) =>
                                          setNewSnapshotTitle(event.target.value)
                                        }
                                        onKeyDown={(event) => {
                                    ...
                                    <div className="flex max-h-36 flex-col gap-0.5 overflow-y-auto">
                                      {savedSnapshots.map((snapshot) => (
                                        <button
                                          ...
                                          onClick={() =>
                                            void updateExistingSnapshot(group, snapshot)
                                          }
                                        >
