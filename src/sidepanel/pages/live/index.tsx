...
import {
  C_GROUP_COLOR_BG_CLASSES,
  C_GROUP_COLOR_BORDER_CLASSES,
} from '@/constants/group-color-classes'
...
function LiveManagement() {
  ...
  const toggleGroup = async (groupId: number) => {
    ...
    if (groupId === EMockGroup.PINNED || groupId === EMockGroup.UNGROUP) {
      if (isCollapsed) {
        setAccordionValues((prev) => prev.filter((id) => id !== String(groupId)))
      } else {
      ...
      <Accordion type="multiple" className="w-full" value={accordionValues}>
        {tabsPinned.length ? (
          <AccordionItem value={`${EMockGroup.PINNED}`}>
            <AccordionTrigger
              onClick={() => toggleGroup(EMockGroup.PINNED)}
              className="py-2 mt-1 pl-2"
            >
        ...
        {groups.map((group) => (
          <AccordionItem key={group.id} value={`${group.id}`}>
            ...
            >
              <Badge className={cn('text-white', C_GROUP_COLOR_BG_CLASSES[group.color])}>
                {group.title}
              </Badge>
              <Button
                variant="ghost"
                size="sm"
                className="size-5 ml-auto cursor-default hover:bg-gray-200"
                onClick={(e) => {
                  e.stopPropagation()
                  closeGroup(group)
                }}
            ...
            <AccordionContent className={cn('flex flex-col text-balance')}>
              {group.tabs.map((tab) => (
                <TabListItem key={tab.id} tab={tab} />
              ))}
            </AccordionContent>
        ...
        {tabsUngroup.length ? (
          <AccordionItem value={`${EMockGroup.UNGROUP}`}>
            <AccordionTrigger
              onClick={() => toggleGroup(EMockGroup.UNGROUP)}
              className="py-2 mt-1 pl-2"
            >
              {MOCK_GROUP[EMockGroup.UNGROUP]}
            </AccordionTrigger>
            <AccordionContent className="flex flex-col text-balance">
              {tabsUngroup.map((tab) => (
                <TabListItem key={tab.id} tab={tab} />
